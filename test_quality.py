...
from evaluation.dataset import C4Dataset, HumanEvalDataset, WMT16DE_ENDataset, CNN_DailyMailDataset
...
from evaluation.tools.text_quality_analyzer import PPLCalculator, LogDiversityAnalyzer, BLEUCalculator, PassOrNotJudger, \
    GPTTextDiscriminator, BERTScoreCalculator
...
def assess_quality(algorithm_name, metric):
    if metric == 'PPL':
        ...
        pipeline = DirectTextQualityAnalysisPipeline(dataset=my_dataset,
                                                     ...
                                                     analyzers=[PPLCalculator(
                                                         model=AutoModelForCausalLM.from_pretrained(
                                                             "Qwen/Qwen2.5-3B-Instruct", device_map='cuda',torch_dtype=torch.bfloat16),
        ...
        opt_model = AutoModelForCausalLM.from_pretrained(
            "facebook/opt-1.3b",
            device_map='cuda',  # 替换原来的 .to(device)
            torch_dtype=torch.bfloat16  # 开启半精度
        )
        ...
        transformers_config = TransformersConfig(
            model=AutoModelForSeq2SeqLM.from_pretrained("facebook/nllb-200-distilled-600M").to(
                device),
            tokenizer=tokenizer,
            device=device,
            vocab_size=256206,
        ...
        # Using the same translation dataset as BLEU, since BERTScore requires a reference text
        my_dataset = CNN_DailyMailDataset('dataset/cnn_dailymail/test-00000-of-00001.jsonl')

        # Load the tokenizer and transformers config for text generation
        tokenizer = AutoTokenizer.from_pretrained(
        ...
        # Initialize the Referenced Text Quality Pipeline
        # Note: quality_analysis.py defines the parameter as 'analyzers' (a list)
        pipeline = ReferencedTextQualityAnalysisPipeline(
            dataset=my_dataset,
            watermarked_text_editor_list=[],
            unwatermarked_text_editor_list=[],
            analyzers=[BERTScoreCalculator(model_path="roberta-large")],
            # Replace "roberta-large" with your local model path if needed
            unwatermarked_text_source='generated',
            show_progress=True,
            return_type=QualityPipelineReturnType.MEAN_SCORES
        )
