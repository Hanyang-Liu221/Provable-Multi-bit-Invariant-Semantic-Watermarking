    ...
    def evaluate(self, watermark: BaseWatermark):
        ...
        for index in range(5):
            ...
            for analyzer in self.analyzers:
                prepared_data = self._prepare_input_for_quality_analyzer(edited_watermarked_text,
                                                                         edited_unwatermarked_text, index)
        ...
        if self.return_type == QualityPipelineReturnType.FULL:
            ...
            if evaluation_result:
                ...
                for analyzer_name in analyzer_names:
                    # 将所有分数值提取成列表
                    w_scores = [result.watermarked_quality_scores[analyzer_name] for result in evaluation_result]
                    u_scores = [result.unwatermarked_quality_scores[analyzer_name] for result in evaluation_result]

                    # 计算 watermarked 的各项统计指标
                    watermarked_stats[analyzer_name] = {
                        'mean': np.mean(w_scores),
                        'median': np.median(w_scores),
                        'q1': np.percentile(w_scores, 25),
                        'q3': np.percentile(w_scores, 75),
                        'max': np.max(w_scores)  # 异常值最高点
                    }

                    # 计算 unwatermarked 的各项统计指标
                    unwatermarked_stats[analyzer_name] = {
                        'mean': np.mean(u_scores),
                        'median': np.median(u_scores),
                        'q1': np.percentile(u_scores, 25),
                        'q3': np.percentile(u_scores, 75),
                        'max': np.max(u_scores)
                    }

            return {
                'watermarked': watermarked_stats,
                'unwatermarked': unwatermarked_stats
            }
